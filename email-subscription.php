...
//	get_footer();
	...
	$orig = $resp['origin'];
	$home = home_url();
	print("<!DOCTYPE html>\n<html><head><title>Email Subscription</title></head>" .
		"<body><h1>$msg</h1><p>You can now go <a href=\"$orig\">back</a> or to the <a href=\"$home\">start page</a>.</p></body></html>");
	exit();
}
...
function emailSub_ajaxCallback() {
	...
	emailSub_ajaxResponse(array(
		'status'=>200,
		'message'=>$msg,
		'origin'=>isset($_GET['current_page'])?htmlspecialchars($_GET['current_page']):''
	), $ajaxResponse);
}
//register callback
